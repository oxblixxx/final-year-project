...
resource "aws_route_table" "private_subnet_main_0" {
  vpc_id = module.vpc.vpc_id
...
resource "aws_route_table" "private_subnet_main_1" {
  vpc_id = module.vpc.vpc_id
...
resource "aws_route_table" "private_subnet_main_2" {
  vpc_id = module.vpc.vpc_id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = module.vpc.natgw_ids[1]
  }
}

resource "aws_route_table" "private_subnet_main_3" {
  vpc_id = module.vpc.vpc_id
...
resource "aws_route_table_association" "private_subnet_directory_1" {
  ...
  route_table_id = aws_route_table.private_subnet_main_0.id
}
...
resource "aws_route_table_association" "private_subnet_directory_2" {
  ...
  route_table_id = aws_route_table.private_subnet_main_2.id
}

resource "aws_route_table_association" "private_subnet_workspace_1" {
  # Access the subnet ID dynamically using the index
  subnet_id = module.vpc.private_subnets[2]
  route_table_id = aws_route_table.private_subnet_main_1.id
}
